...
public class Carpet implements CommandExecutor {
    ...
    public boolean onCommand(final CommandSender sender, final Command command, final String label, final String[] args) {
        ...
        final net.digiex.magiccarpet.Carpet carpet = MagicCarpet.getCarpets().getCarpet(player);
        if (carpet == null) {
        ...
        if (args.length < 1) {
            ...
            if (carpet.isVisible()) {
                try {
                    ...
                    word = word.trim();
                    final Material m = Material.matchMaterial(word);
                    if (m != null) {
                        carpet.changeCarpet(m, d);
                        return true;
                    } else if (carpet.getData() != d && carpet.canHaveData(carpet.getThread())) {
                        carpet.setData(d);
                        return true;
                    } else {
